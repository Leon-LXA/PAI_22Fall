...
def log_posterior_probs(x):
    ...
    prob_norm = np.sum(np.log(HYPOTHESIS_SPACE[0].pdf(x)))+np.log(PRIOR_PROBS[0])
    prob_laplace = np.sum(np.log(HYPOTHESIS_SPACE[1].pdf(x))) + np.log(PRIOR_PROBS[1])
    prob_t = np.sum(np.log(HYPOTHESIS_SPACE[2].pdf(x))) + np.log(PRIOR_PROBS[2])

    log_p = np.empty(3,)
    # assert log_p.shape == (3,)
    # prob_total = prob_t*prob_norm*prob_laplace

    log_p[0] = prob_norm
    log_p[1] = prob_laplace
    log_p[2] = prob_t
