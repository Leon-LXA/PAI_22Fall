...
class Model(object):
    ...
    def __init__(self):
        ...
        self.params = {"l": 0.05, "sigma_f": 0.3}
    ...
    def make_predictions(self, test_features: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # print(Kff)
        Kyy = self.gaussian_kernel(test_features, test_features)
        Kfy = self.gaussian_kernel(self.train_X, test_features)
        Kff_inv = np.linalg.inv(Kff + 1e-10 * np.eye(len(self.train_X)))

        gp_mean = Kfy.T.dot(Kff_inv).dot(self.train_Y) * 1.04
        gp_std = Kyy - Kfy.T.dot(Kff_inv).dot(Kfy)

        # kernel = RBF() + ConstantKernel(constant_value=0.01)
        # gpr = GaussianProcessRegressor(kernel=kernel, alpha=5, random_state=0).fit(self.train_X, self.train_Y)
        # gp_mean, gp_std = gpr.predict(test_features, return_std=True)
...
def perform_extended_evaluation(model: Model, output_dir: str = '/results'):
    ...
    print(visualization_xs)
    print(visualization_xs.shape[0])
...
def main():
    ...
    # x = np.array([[700, 500], [800, 600], [1029, 750]])
