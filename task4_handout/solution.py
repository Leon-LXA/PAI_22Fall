...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for i in range(nLayer):
        layers.append(nn.Linear(sizes[1 + i], sizes[1 + i]))
        layers.append(activation())
...
class Actor(nn.Module):
    ...
    def _distribution(self, obs):
        ...
        # distribution.
        # print(obs)
        # print(self.logits_net(obs))
        # print(Categorical(logits=self.logits_net(obs)))
        # input()
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi = self.actor._distribution(state) # 四个action对应的概率
            a = pi.sample() # 对应四个概率来采样action
            logp_a = self.actor._log_prob_from_distribution(pi, a) #在状态s下，使用策略pi采取动作a的log概率
            v = self.critic(state)
    ...
    def get_action(self, obs):
        ...
        action = self.act(torch.Tensor(obs))
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        act_loss = - tdres @ agent.actor.forward(obs, act)[1] # TD残差和logp_a的乘积
        act_loss.backward()
