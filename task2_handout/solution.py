...
def calc_calibration_curve(predicted_probabilities: np.ndarray, labels: np.ndarray, num_bins=30)-> dict:
    ...
    for bin_i, (bin_lower, bin_upper) in enumerate(zip(bin_lowers, bin_uppers)):
        ...
        for i in range(len(index)):
            freq += accuracies[i]
        if len(index):
            calib_accuracy[bin_i] = freq/len(index)
        else:
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.decay = 0.7
        self.optimizer = torch.optim.SGD(self.network.parameters(), lr=self.learning_rate, weight_decay=self.decay)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                out = F.softmax(out, 1)
                l = 2
                tau = self.network.dropout_p * (l ** 2) / 2 / self.batch_size / self.decay
                loss = F.nll_loss(F.log_softmax(out, dim=1), batch_y, reduction='sum') / tau / self.batch_size
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        # self.dropout_at_eval = True
        estimated_probability = torch.zeros(x.shape[0], 10)
        for i in range(num_sample):
            temp = F.softmax(self.network(x), dim=1)
            # print(temp)
            estimated_probability += temp
